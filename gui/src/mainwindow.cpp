...
void MainWindow::onSettings() {
    SettingsDialog dlg(appSettings, this);
    connect(&dlg, &SettingsDialog::chatFontChanged, this, &MainWindow::onChatFontChanged);
    connect(&dlg, &SettingsDialog::cliPathChanged, this, [this](const QString &path) {
        backend->setCliPath(path);
    });
    dlg.exec();
}

void MainWindow::onChatFontChanged(const QFont &font) {
    chatFont = font;
    chatView->setFont(chatFont);
}

void MainWindow::handleFileOffer(const QString &sender, const QString &filename, const QString &sizeStr) {
    bool autoAccept = appSettings->value("privacy/autoAcceptFiles", false).toBool();

    if (autoAccept) {
        /* Auto-accept: save to Downloads */
        backend->sendMessage("", "/accept");
        return;
    }

    /* Show acceptance dialog */
    QMessageBox msgBox(this);
    msgBox.setWindowTitle("Incoming File");
    msgBox.setText(QString("%1 wants to send you a file:\n\n\"%2\" (%3)\n\nAccept?")
                   .arg(sender, filename, sizeStr));
    msgBox.setIcon(QMessageBox::Question);
    QPushButton *acceptBtn = msgBox.addButton("Accept", QMessageBox::AcceptRole);
    QPushButton *saveAsBtn = msgBox.addButton("Save as...", QMessageBox::ActionRole);
    ...
    if (msgBox.clickedButton() == acceptBtn) {
        ...
        if (!savePath.isEmpty()) {
            backend->sendMessage("", "/accept " + savePath);
        } else {
            backend->sendMessage("", "/reject");
        }
    } else {
        backend->sendMessage("", "/reject");
    }
...
void MainWindow::onNewMessages(const QStringList &messages) {
    for (const QString &m : messages) {
        /* Intercept [FILE_OFFER] messages for acceptance dialog */
        static QRegularExpression fileOfferRe(
            "^\\[FILE_OFFER\\]\\s+(\\S+)\\s+wants to send\\s+\"([^\"]+)\"\\s+\\(([^)]+)\\)");
        QRegularExpressionMatch fom = fileOfferRe.match(m);
        if (fom.hasMatch()) {
            handleFileOffer(fom.captured(1), fom.captured(2), fom.captured(3));
            appendChatLine(m);
            continue;
        }
    ...
    if (isHidden && !messages.isEmpty()) {
        ...
        if (trayIcon && trayIcon->isVisible()) {
            bool showContent = appSettings->value("privacy/showNotificationContent", false).toBool();
            QString notificationText;

            if (showContent && messages.count() == 1) {
                /* Show message content in notification */
                QString msg = messages.first();
                /* Strip timestamp and tags for cleaner notification */
                static QRegularExpression tsRe("^\\s*\\[\\d{2}:\\d{2}:\\d{2}\\]\\s*");
                msg.remove(tsRe);
                if (msg.length() > 100) {
                    msg = msg.left(100) + "...";
                }
...
void MainWindow::appendChatLine(const QString &line) {
    // Parse lines with verification prefix: [16:54:43] [V] Admin: message
    static QRegularExpression reVerified("^\\s*\\[(\\d{2}:\\d{2}:\\d{2})\\]\\s*\\[([VT?!])\\]\\s*([^:]+):\\s*(.*)$");
    QRegularExpressionMatch mv = reVerified.match(line);

    if (mv.hasMatch()) {
        QString ts = mv.captured(1);
        QString verStatus = mv.captured(2);
        QString sender = mv.captured(3).trimmed();
        QString msg = mv.captured(4).trimmed();

        QString verIcon;
        if (verStatus == "V") {
            verIcon = "<span style='color:#2ecc71' title='Verified'>[V]</span>";
        } else if (verStatus == "T") {
            verIcon = "<span style='color:#3498db' title='TOFU trusted'>[T]</span>";
        } else if (verStatus == "?") {
            verIcon = "<span style='color:#f39c12' title='No signature'>[?]</span>";
        } else if (verStatus == "!") {
            verIcon = "<span style='color:#e74c3c; font-weight:bold; background:#ffdddd' title='WARNING: Signature failed or key changed — possible MITM attack!'>[!!! KEY MISMATCH]</span>";
        }

        QString html = QString("<span style='color:gray'>[%1]</span> %2 <b>%3:</b> %4")
                           .arg(ts.toHtmlEscaped(), verIcon, sender.toHtmlEscaped(), msg.toHtmlEscaped());
        chatView->append(html);
        return;
    }

    // Parse lines without verification prefix: [16:54:43] Admin: message
    ...
    if (m.hasMatch()) {
        ...
        chatView->append(html);
    } else if (line.startsWith("[FILE_OFFER]")) {
        QString html = QString("<span style='color:#3498db; font-weight:bold'>%1</span>")
                           .arg(line.toHtmlEscaped());
        chatView->append(html);
    } else if (line.startsWith("[TOFU]") || line.startsWith("[IDENTITY]") ||
               line.startsWith("[VERIFIED]") || line.startsWith("[WARNING]")) {
        // Identity/TOFU status messages
        QString color = "#3498db"; // blue default
        if (line.startsWith("[WARNING]")) color = "#e74c3c"; // red
        else if (line.startsWith("[TOFU]")) color = "#f39c12"; // orange
        else if (line.startsWith("[VERIFIED]") || line.startsWith("[IDENTITY]")) color = "#2ecc71"; // green

        QString html = QString("<span style='color:%1'>%2</span>").arg(color, line.toHtmlEscaped());
        chatView->append(html);
    } else {
...
void MainWindow::createMenus() {
    ...
    QAction *genIdentity = new QAction("Generate identity key", this);
    connect(genIdentity, &QAction::triggered, this, [this]() {
        if (backend->hasIdentity()) {
            if (QMessageBox::question(this, "Identity",
        ...
        if (backend->generateIdentity()) {
            QMessageBox::information(this, "Identity",
                "Identity key generated.\n"
                "Your public key has been copied to clipboard.");
        }
    });
    keysMenu->addAction(genIdentity);

    QAction *manageKeysAct = new QAction("Manage trusted keys", this);
    connect(manageKeysAct, &QAction::triggered, this, [this]() {
        KnownKeysDialog dlg(this);
        dlg.exec();
    });
    keysMenu->addAction(manageKeysAct);
