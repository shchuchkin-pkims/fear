...
**Реализовано** (`identity/rotation.c`, Phase C-2), формат одной записи bundle — 136 байт:

```
entry   = [recipient_pk(32)][nonce(24)][ct(80)]
ct      = crypto_box(K_room_new || binding, nonce, X25519(recipient_pk), X25519(sender_sk))
binding = BLAKE2b("fear.rotation.v1" || room_id || new_version(2, LE) || sender_pk || recipient_pk, 32)
```

`crypto_box` аутентифицирует отправителя (получатель знает, что запись сделал именно `sender_pk`), а `binding` внутри шифртекста привязывает запись к одной комнате, одному поколению K_room и одному получателю: переигранная в другую комнату, в другую версию или в чужой слот запись не открывается. Право отправителя ротировать комнату — вопрос политики вызывающего кода (список доверенных участников), а не этого модуля. Тест-векторы (PyNaCl) заморожены в `tests/test_rotation.c`.
