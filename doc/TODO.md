...
- [ ] **База Android открывается с fallbackToDestructiveMigration.** Любое
      изменение схемы сотрёт местную историю и контакты. Сегодня это обошли -
      новому выводу идентификатора колонка не понадобилась, - но следующая
      правка схемы так не обойдётся. Нужна настоящая миграция.
- [~] **Phase D - приватность метаданных.** Сделана половина: комната на
      проводе больше не название, а BLAKE2b от него. Оператор не читает в
      своём журнале, кто в какой комнате сидит, и заодно исчезла приставка
      «pm:», по которой личные комнаты отличались от общих с одного взгляда.
      Вход по имени работает как раньше - обе стороны хешируют одну строку.
      ...
      Остаётся: отображаемое имя тоже едет по проводу открытым. Перенести его
      внутрь шифра - отдельная работа, и немаленькая: имя сегодня несущее.
      На нём держится защита от подмены отправителя на сервере, из него
      строится список участников комнаты, по нему же ведётся реестр для
      ротации ключа. Замена - случайная метка сессии на проводе и настоящее
      имя внутри запечатанного сообщения; сервер сохраняет защиту от подмены
      (метка закреплена за соединением), список участников становится
      счётчиком, а кто есть кто клиенты и так узнают из анонсов личности.
      Делать это надо цельным куском на обеих платформах, с живой проверкой:
      наполовину внесённая правка ломает и показ имён, и ротацию.
