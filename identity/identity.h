...
/** Длина метки сессии: 22 знака base64url от 16 байт + NUL. */
#define IDENTITY_SESSION_TAG_LEN 23
 ...
 *
 * Настоящее имя уезжает внутрь шифра, в анонс личности, и вместе с ним -
 * подпись, которая привязывает имя к этой самой метке.
 */
 ...
 *
 * Подпись только над именем позволила бы взять чужой анонс и повторить его
 * под своей меткой - имя досталось бы вместе с ним. Метка внутри подписи
 * это закрывает: она привязывает «я зовусь так» к «я вот это соединение».
 *
 * Метка постоянной длины, поэтому склейка читается однозначно и без
 * разделителя.
 *
 * @return длина, уложенная в out, или 0 если не поместилось
 */
